...
build_output_dir() {
  local method="$1"
  local method_root="$ROOT/outputs/$DATASET/llama3_8b/${method}"
  local progress_dir=""
...
for method in $METHODS; do
  ...
  # beam=8, batch=12 speeds up LLM CO generation while preserving checkpoint resume.
  ...
  if [[ -n "$REUSE_STAGE1_FROM" ]]; then
    REUSE_SOURCE="$REUSE_STAGE1_FROM"
  elif [[ "$method" != "ilp" && -n "$LAST_STAGE1_TRACE" ]]; then
    REUSE_SOURCE="$LAST_STAGE1_TRACE"
  fi
  if [[ -n "$REUSE_SOURCE" ]]; then
    if [[ ! -f "$REUSE_SOURCE" ]]; then
      echo "[run_methods] missing reuse source for $method: $REUSE_SOURCE" >&2
      exit 3
    fi
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    ...
    if [[ -n "$REUSE_SOURCE" ]]; then
      ...
      if [[ "$method" == "ilp" ]]; then
        LAST_STAGE1_TRACE="$REUSE_SOURCE"
      fi
    elif [[ "$method" == "ilp" ]]; then
      ...
      echo "[run_methods] dry-run missing reuse source for $method" >&2
      exit 3
    fi
    continue
  fi
