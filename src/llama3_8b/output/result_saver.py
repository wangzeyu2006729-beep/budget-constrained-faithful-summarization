...
def _save_default_results(result_file, metrics, generated_summaries, references, config_header, all_sample_logs=None):
    ...
    with open(result_file, "w", encoding="utf-8") as f:
        ...
        if totals is None and rouge_keys:
            f.write(f"ROUGE Scores (F1 only, impl={rouge_impl}):\n")
            for key in rouge_keys:
                f.write(f"  {key:8s}  F1={rouge_scores[key]:.2f}%\n")
        elif rouge_keys:
        ...
        f.write(f"  Precision={metrics['bert_P']:.2f}%  Recall={metrics['bert_R']:.2f}%  F1={metrics['bert_F']:.2f}%\n")

        f.write("\nFactCC (manueldeprada/FactCC):\n")
        f.write(f"  SentenceAvgCorrect: {metrics['factcc']:.2f}%\n")

        f.write("\nMiniCheck (EMNLP 2024, MiniCheck-RoBERTa-Large):\n")
        f.write(f"  SummaryAvgConsistent: {metrics['minicheck']:.2f}%\n")
        ...
        if all_sample_logs:
            ...
            for j in range(min(10, num_samples)):
                ...
                f.write(f"(f) Final summary: {generated_summaries[j]}\n")
