...
DRY_RUN="${DRY_RUN:-0}"
CO_BUDGET_SENTENCES_OVERRIDE="${CO_BUDGET_SENTENCES:-${BUDGET_SENTENCES:-8}}"
...
build_output_dir() {
  local method="$1"
  local method_root="$ROOT/outputs/$DATASET/primera_multinews/${method}"
  local progress_dir=""
...
for method in $METHODS; do
  ...
  CO_GEN_BATCH="${CO_GENERATION_BATCH_SIZE:-2}"
