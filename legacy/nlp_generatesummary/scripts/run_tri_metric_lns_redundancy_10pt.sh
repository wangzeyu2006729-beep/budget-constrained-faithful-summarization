...
WEIGHTS_FILE="${WEIGHTS_FILE:-$ROOT/scripts/tri_metric_lns_redundancy_weights_10pt.csv}"
GRID_CSV="${GRID_CSV:-$ROOT/bart/results/tri_metric_grid_search_lns_redundancy_10pt_${SPLIT}_n${NUM_SAMPLES}.csv}"
GRID_RUN_ROOT="${GRID_RUN_ROOT:-$ROOT/bart/results/tri_metric_grid_runs_lns_redundancy_10pt_${SPLIT}_n${NUM_SAMPLES}}"

read -r -a PAPER_METRIC_ARGS <<< "$PAPER_METRICS"
read -r -a EXTRA_METRIC_ARGS <<< "$EXTRA_METRICS"

echo "=== LNS Tri-Metric Search With Redundancy ==="
echo "SPLIT=$SPLIT"
