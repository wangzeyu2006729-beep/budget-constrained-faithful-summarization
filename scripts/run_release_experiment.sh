...
usage() {
  cat <<'EOF'
Usage:
  scripts/run_release_experiment.sh --model MODEL --method METHOD [options] [-- extra run.py args...]
...
Options:
  ...
  --beam-size N              default: paper-aligned per model/method
  --budget-sentences N       supported for PRIMERA and instruction-tuned LLM runners
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --split)
      ...
      ;;
    --num-samples)
      NUM_SAMPLES="${2:?missing value for --num-samples}"
      shift 2
      ;;
    --sample-mode)
      SAMPLE_MODE="${2:?missing value for --sample-mode}"
      shift 2
      ;;
    --sample-seed)
      SAMPLE_SEED="${2:?missing value for --sample-seed}"
      shift 2
      ;;
    --beam-size)
      BEAM_SIZE="${2:?missing value for --beam-size}"
      shift 2
      ;;
    --budget-sentences)
      BUDGET_SENTENCES="${2:?missing value for --budget-sentences}"
      shift 2
      ;;
    --output-tag)
      OUTPUT_TAG="${2:?missing value for --output-tag}"
      shift 2
      ;;
    --compute-dtype)
      COMPUTE_DTYPE="${2:?missing value for --compute-dtype}"
      shift 2
      ;;
    -h|--help)
      usage
...
case "$MODEL" in
  bart)
    ...
    SUPPORTS_BUDGET=0
    if [ "$DATASET" != "cnn_dailymail" ]; then
      echo "BART release runner supports cnn_dailymail only." >&2
      exit 2
    fi
    ;;
  primera_multinews)
    ...
    SUPPORTS_BUDGET=1
    if [ "$DATASET" != "multi_news" ]; then
      echo "PRIMERA release runner supports multi_news only." >&2
      exit 2
    fi
    ;;
  llama3_8b)
...
if [ -n "$BUDGET_SENTENCES" ]; then
  if [ "$SUPPORTS_BUDGET" = "1" ]; then
    ...
    echo "--budget-sentences is not supported by the BART runner." >&2
    exit 2
  fi
fi
...
printf '[release-runner] root=%s\n' "$ROOT"
printf '[release-runner] output_dir=%s\n' "$OUTPUT_DIR"
printf '[release-runner] command: '
printf '%q ' "${CMD[@]}"
printf '\n'
...
if [ "${DRY_RUN:-0}" = "1" ]; then
  ...
  echo "[release-runner] dry run only; wrote $MARKER"
  exit 0
fi
